...
export default function CreateLesson() {
    ...
    useEffect(() => {
        ...
        const fetchLesson = async () => {
            try {
                ...
                setMainMediaType(data.mainMediaType || "NONE");
                setMainMediaUrl(data.mainMediaUrl || "");
                setSelectedQuizId(data.quizId || null);
                setAdditionalResources(
                    data.additionalResources.length > 0
                        ? data.additionalResources.map((res) => ({ title: res.title, url: res.url }))
                        : [{ title: "", url: "" }]
                );
            } catch (err) {
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError("Помилка під час збереження уроку.");
        }
    };
        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-8 py-8 flex flex-col items-center gap-8 relative">
            <section className="text-center max-w-3xl w-full">
                ...
                <p className="text-base sm:text-lg text-foreground-secondary">
                    Додайте, або змініть навчальний матеріал, мету та зміст для вашого уроку
                </p>
            </section>

            <form onSubmit={handleUpdate} className="w-full max-w-4xl flex flex-col gap-6">
