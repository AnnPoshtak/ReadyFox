...
export const AuthPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(formatErrorMessage(backendMessage));
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className="w-full max-w-md bg-surface border-2 border-outline/60 rounded-3xl p-6 sm:p-8 shadow-xl shadow-shadow/10 relative z-10 animate-in fade-in zoom-in-95 duration-500">
            ...
            <div className="flex bg-brand-soft/50 p-1 rounded-2xl mb-6 border border-outline/40">
              <button
                ...
                className={`flex-1 py-2 rounded-xl font-heading font-bold text-xs sm:text-sm transition-all duration-300 cursor-pointer ${!isLogin
            ...
            <form onSubmit={handleSubmit} className="space-y-4 transition-all duration-200">
              {!isLogin && (
              ...
              <div className={`overflow-hidden transition-all duration-200 ${errorMessage ? "max-h-20 opacity-100 mb-2" : "max-h-0 opacity-0 mb-0"}`}>
